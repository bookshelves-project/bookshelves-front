<template>
  <section aria-labelledby="book-main">
    <div class="space-y-8 lg:grid lg:gap-x-4 lg:gap-y-8 lg:space-y-0 lg:grid-cols-2 pb-6">
      <dl class="space-y-8 md:grid md:gap-x-4 md:gap-y-8 md:space-y-0 md:grid-cols-2">
        <div v-if="book.authors" class="sm:col-span-1">
          ...
          <dd class="mt-1 text-sm text-gray-900 dark:text-gray-100">
            <h2>
              <span v-for="(author, authorId) in book.authors" :key="authorId" class="mr-1">
                <nuxt-link
                  :to="
                    localePath({
                      name: 'authors-slug',
        ...
        <div v-if="book.serie" class="sm:col-span-1">
          <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Serie</dt>
          <dd class="mt-1 text-sm text-gray-900 dark:text-gray-100">
            <h2>
              <nuxt-link
                :to="
        ...
        <div v-if="book.publisher" class="sm:col-span-1">
          <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Publishing</dt>
          <dd class="mt-1 text-sm text-gray-900 dark:text-gray-100">
            <nuxt-link
              :to="
                localePath({
        ...
        <div v-if="book.publishDate" class="sm:col-span-1">
          ...
          >{{ $formatDate(book.publishDate) }}</dd>
        </div>
        <div v-if="book.language" class="sm:col-span-1">
          ...
          <dd class="mt-1 text-sm text-gray-900 dark:text-gray-100">
            <nuxt-link
              ...
              "
              class="internal-link"
            >{{ formatLanguage(book.language).label }}</nuxt-link>
          </dd>
        </div>
        <div v-if="book.identifier" class="sm:col-span-1">
          <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Identifiers</dt>
          <dd
            v-if="book.identifier.isbn || book.identifier.isbn13"
            class="mt-1 text-sm text-gray-900 dark:text-gray-100"
          >
            <ul>
              <li v-if="book.identifier.isbn">ISBN: {{ book.identifier.isbn }}</li>
              <li v-if="book.identifier.isbn13">ISBN13: {{ book.identifier.isbn13 }}</li>
            </ul>
          </dd>
        </div>
        <div v-if="book.pageCount" class="sm:col-span-1">
          <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Page count</dt>
          <dd
        ...
        <div v-if="book.genres.length >= 1" class="sm:col-span-1">
          ...
          <dd class="flex flex-wrap mt-1 text-sm text-gray-900 dark:text-gray-100">
            <span v-for="(genre, genreId) in book.genres" :key="genreId">
              ...
              <span v-if="genreId !== book.genres.length - 1" class="mr-1">,</span>
            </span>
          </dd>
        </div>
