<template>
  <div class="max-w-lg w-full lg:max-w-xs flex search-bar">
    <app-modal :open="modalOpened" size="xl" @close="(data) => setModalOpened(data)">
    ...
    >
      ...
      <div
        class="absolute cursor-text inset-0 border border-gray-300 dark:border-gray-600 hover:border-2 hover:dark:border-primary-400 hover:border-primary-600 transition-colors duration-75 rounded-md"
      ></div>
      <input id="search" class="search" placeholder="Search" type="search" name="search" />
      <div class="absolute top-1/2 transform -translate-y-1/2 right-1.5">
        <span
          style="opacity: 1"
          class="hidden sm:block text-gray-400 text-sm leading-5 py-0.5 px-1.5 border border-gray-300 dark:border-gray-600 rounded-md"
        >
